...
export async function requireBusinessAccess(req: Request, businessId: string): Promise<{ userId: string } | Response> {
  ...
  // Internal calls (the zatca-sweep cron) authenticate with the service-role
  // key — no user context. Trusted: only holders of the service key can do this.
  const bearer0 = authHeader.replace(/^Bearer\s+/i, "");
  if (bearer0 && bearer0 === Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")) return null;
  const _ik = Deno.env.get("INTERNAL_KEY");
  if (bearer0 && _ik && bearer0 === _ik) return null;
    ...
    <cac:AccountingCustomerParty>
        <cac:Party>
            ${(o.buyer.street || o.buyer.city) ? `<cac:PostalAddress>
                ...
                <cac:Country><cbc:IdentificationCode>${xesc(o.buyer.country || "SA")}</cbc:IdentificationCode></cac:Country>
            </cac:PostalAddress>
...
Deno.serve(async (req) => {
  ...
  const { data: items, error: itemsErr } = await db.from("invoice_items").select("*").eq("invoice_id", inv.id).order("sort_order");
  if (itemsErr) return json({ error: "items_query_failed", detail: itemsErr.message }, 422);
  if (!items?.length) return json({ error: "invoice_has_no_items", invoice_id: inv.id }, 422);
  ...
  // ── Build UBL ──
  /* Simplified = B2C receipts only. B2B AND EXPORT invoices are STANDARD
     (0100000): exports carry buyer details and zero-rated category Z lines. */
  const isSimplified = inv.invoice_type === "b2c";
  ...
  const xml = buildInvoiceXml({
    ...
    pih,
    seller: await (async () => {
      // Base: the business (legal name + TRN never change — a branch is not a
      // separate legal entity). If the invoice was issued by a BRANCH, its own
      // CR number and national address override per-field (ZATCA requires each
      // branch's CR + address on its invoices); missing fields fall back.
      const base = {
      ...
      if (inv.branch_id) {
        const { data: br } = await db.from("branches").select("*").eq("id", inv.branch_id).maybeSingle();
        if (br) {
          if (br.cr_number) base.crn = String(br.cr_number);
          if (br.street_name) base.street = String(br.street_name);
          if (br.building_number) base.building = String(br.building_number);
          if (br.city) base.city = String(br.city);
          if (br.district) base.district = String(br.district);
          if (br.postal_code) base.postal = String(br.postal_code);
        }
      }
      return base;
    })(),
    buyer: {
      ...
      country: inv.buyer_country ? String(inv.buyer_country) : undefined,
    },
    lines,
  });
  ...
  // Columns match the LIVE zatca_submissions schema (zatca_response /
  // response_status / submission_type — the original richer design).
  const respBody = typeof resp.body === "object" && resp.body !== null ? resp.body : { raw: resp.raw };
  const vr = (respBody as Record<string, any>)?.validationResults;
  const pickMsgs = (arr: unknown) => Array.isArray(arr) ? arr.map((m: any) => `${m?.code ?? ""}: ${m?.message ?? ""}`.trim()) : null;
  const { data: sub, error: subErr } = await db.from("zatca_submissions").insert({
    ...
    pih,
    submission_type: env === "sandbox" ? "compliance" : (isSimplified ? "reporting" : "clearance"),
    invoice_subtype: isSimplified ? "simplified" : "standard",
    response_status: resp.status,
    zatca_response: respBody,
    warnings: pickMsgs(vr?.warningMessages),
    errors: pickMsgs(vr?.errorMessages),
  }).select("id").maybeSingle();
  if (subErr) console.error("[zatca-submit] submissions archive failed:", subErr.message);
